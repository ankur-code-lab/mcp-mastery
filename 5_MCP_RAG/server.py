...
- Database info
- Uses google/embeddinggemma-300m from HuggingFace via sentence-transformers
"""
...
class EmbeddingGemmaEmbeddings(Embeddings):
    """LangChain-compatible wrapper for google/embeddinggemma-300m via sentence-transformers."""
    ...
    def embed_documents(self, texts: List[str]) -> List[List[float]]:
        """Embed a list of documents using encode_document."""
        embeddings = self.model.encode_document(texts)
        return embeddings.tolist()

    def embed_query(self, text: str) -> List[float]:
        """Embed a single query using encode_query."""
...
embeddings = EmbeddingGemmaEmbeddings(model_name=EMBED_MODEL)
