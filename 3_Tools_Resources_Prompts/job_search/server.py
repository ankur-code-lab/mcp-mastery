...
from pypdf import PdfReader  # For extracting text from PDF resumes
...
@mcp.tool()
async def search_jobs(role: str, location: str, max_results: int = 5) -> List[Dict]:
    """
    ...
    # Step 8: Summarize job description for each job
    results = []
    for job in job_list:
...
@mcp.tool()
async def save_job(job_id: str, salary: Optional[str] = None) -> str:
    """
    ...
    # Step 4: Find the job with the specified job_id (case-insensitive)
    selected = next((job for job in jobs if job.get("job_id", "").lower() == job_id.lower()), None)
    if not selected:
...
async def candidate_resume() -> str:
    """
...
@mcp.prompt()
async def analyze_job_market(role: str, location: str, num_jobs: int = 5) -> str:
    """
...
@mcp.prompt()
async def personalized_job_recommender() -> str:
    """
...
@mcp.prompt()
async def create_match_report() -> str:
    """
